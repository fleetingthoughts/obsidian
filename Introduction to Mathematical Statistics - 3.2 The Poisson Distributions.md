...
It can be shown that given a random variable $X_t$ with parameter $t$ denote the # of occurences within an interval $(0,t]$ with the  pmf $P(X_t=k)=g(k,t)$, then $X_t$ has the Poisson distribution has these 3 properties:
...
The book then shows how we can set up this assumption to derive the $p(x=0)=e^{-\lambda}$ which is the probability of the Poisson distribution when no event occurs. It does so by setting up a differential equation and showing that the probability of no event occurring in $(0,t+h]$, that is, no event occurring in $(0,t]$ and $(t,t+h]$ (or by the stationary principle $g(0,t+h)=g(0,t)g(0,h)$ since we can shift the start time). 