...
1) <u>Homogeneity property (function maintains identical properties throughout time)</u>: $g(1,h) = \lambda h + o(h)$. Colloquially, this says the probability of one and only one occurrence scales linearly with time or whatever $h$ parameter and this remains constant throughout time.. The $o(h)$ notation is an error term that has $\frac{o(h)}{h} \to 0$  as $h \to 0$, that is, $$\lim_{ h \to 0 } \frac{g(1,h)}{h}=\lambda$$Intuitively, what this means is that the probability a singular event occurring is directly proportional to some $\lambda$ constant with respect to $h$ (which can represent something like time)
