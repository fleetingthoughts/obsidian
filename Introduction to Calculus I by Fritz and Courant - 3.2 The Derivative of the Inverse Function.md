...
As we know the derivative of the logarithm by its definition as an indefinite integral and the application of the fundamental theorem of calculus, we can then determine the derivative of the exponential function. 

We can continue to determine the derivative of an exponential with respect to any base. We first recall the inverse function: $$x=\log_{a}y=\frac{\ln y}{\ln a}$$
...
# Theorems
- ***Derivative of the inverse of a function (p.207)*** If a function is differentiable in an interval $a<x<b$ and either $f'(x)> 0$ or $f'(x)<0$ throughout the interval, then the inverse function $x=\phi(y)$ also possesses a derivative at every interior point of its interval definition and satisfies the relationship: $$\frac{dy}{dx}=\frac{1}{\frac{dx}{dy}}$$ 
