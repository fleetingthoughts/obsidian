...
# Assessing Bayesianism by how it models acceptance and Goodman's Riddle of Induction
With the Bayesian model, we observe how this models good science practice of the past where groups of people come to an agreement given strong evidence. The question arises several people can agree on a hypothesis given the same evidence if their prior probabilities are not necessarily equal. Bayesianism places no criteria on prior probabilities being different other than coherence in their system. The question becomes, how do prior probabilities "wash away" and converge to the same answer. The Bayesian answer is in mathematical convergence of probabilities but the convergence in probabilities may be slow. PGS counters by asking what even guarantees that the people will have the same likelihoods if their prior probabilities are not the same. Different people can be shown to believe in varying degrees of the same observed evidence. There are still challenges in showing how the Bayesian model describes how several people can agree on a singular hypothesis, but some even question whether the "washing of priors is even important"
