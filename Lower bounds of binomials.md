...
This lemma is useful for furnishing lower bounds in terms of more manageable polynomials. As an example, this lemma was used in Richard Courant and John Fritz's Calculus book to show that the $\sqrt[n]{p}$ , , $\sqrt[n]{n}$  all tend to 1 as $n$ goes to infinity for some positive $p$ and $\alpha^n$ tends to 0 for $0\le \alpha \lt 1$ . The trick was to rewrite $p$ as a binomial $(1+h_n)$ so that $p = n = (1+h_n)$ where $h_n$ depended on choice of $n$ and show that $h_n$ tends to 0. 
