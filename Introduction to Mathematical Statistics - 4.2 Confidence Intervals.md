...
We now look at the problem of finding the confidence interval in the difference of two different means. We again apply the central limit theorem to produce a statistic with a distribution that does not depend on the parameter. Let $\mu_1$ and $\mu_2$ be the true mean of two populations with their difference denoted: $\triangle = \mu_1-\mu_2$. The sample average of $\triangle$ is denoted $\hat{\triangle}$ is therefore the difference in the sample mean from the realized samples of each population $\hat{\triangle}=\bar{X}-\bar{Y}$. We also note that $$Var(\hat{\triangle})= \frac{\sigma_{1}^2}{n_{1}}+\frac{\sigma_{2}^2}{n_{2}}$$
