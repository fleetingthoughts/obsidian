...
---
We explain 3 distributions related to the Bernoulli distribution and demonstrate their mean and variance. Its important to distinguish the <u>Bernoulli distribution</u> from the <u>Binomial distribution</u>. I often get the terms mixed up but the Binomial distribution is derived from the Bernoulli distribution. We first begin with the Bernoulli distribution.
...
1) Binomial random variable: $X =$ total # of successes in $n$ Bernoulli trials. Requires the parameter $n$ (# of trials) and $p$ (the probability of success) $$p(x) = \binom{n}{x} p^x(1-p)^{n-x} \text{\quad ; \quad} \mu =np \text{\quad ; \quad} Var(X)=np(1-p)=\sigma^2 \text{\quad ; \quad} M(t) =(pe^t+(1-p))^n$$
   The mgf follows from a simple application of the Binomial Theorem when computing $E[e^{tX}]$ and the moments are calculated from the derivatives of the mgf. The cdf of this function is typically not analytical and values are determined computationally.
 

2) Negative Binomial Random Variable: $Y$ = total # of failures before the $r$th success. There will be $r-1$ successes to be permuted in $y+r-1$ trials since the last trial has to be the $r$th success. Note that although it technically has two variables, the # of successes is treated as a fixed parameter. (1 parameter)
