---
tags:
  - math
  - calculus
  - richard_courant
date_created: 2026-05-20
parent: "[[Introduction to Calculus and Analysis I]]"
---
We define terms that will serve as the preliminary foundation for the sense of direction and positiveness for a curve. We then derive the equation of a line on an arbitrary point on the curve that is tangent and normal to the curve; and determine the directional cosines of a curve. All definitions are intuitive as a formal definition will require topology beyond the course of this book. 
1) Intuitive definitions with no rigor:
	- Sense of a curve.
	- Simple closed and simple connected curves.
	- A points angle of inclination.
	- Positive and negative side of a curve.
2) If we given a curve defined by $(x(t),y(t))$, we can restrict an interval such that $x(t)$ is monotonic so that an inverse exists and we can describe a function $y=f(x)$ and therefore a derivative.
3) Define equation of the line tangent to a point on the curve with respect to the derivative of the $x$ and $y$ component.
4) Derive the equation of the normal to a point on the curve with respect to the derivative of the 
5) $x$ and $y$ component.
6) Derive the directional cosines of a curve that parametrizes a curve in polar coordinates.


# Summary of points and nuances
- The sense of a curve is defined by increasing with respect to a chosen reference parameter. If we apply a change in parameter with a monotonically decreasing parameter then the direction with respect to that new parameter is the opposite direction
