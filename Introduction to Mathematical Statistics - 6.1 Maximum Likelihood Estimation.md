...
Now that we have some assurance that the mle method will work eventually, we look at some useful properties of the mle. Namely we answer the question: "if we know the mle of the parameter for a realized sample can we determine the mle for a transformation of the random sample?". Can we determine the mle for $g(\theta)$? We are effectively trying to determine what maximizes $L(g(\theta))$. But note that $g(\theta)$ is just derived from the same sample space as $\theta$ so whatever maximizes $L(\theta)$ is what maximizes $L(g(\theta))$ as well. As ***Theorem 6.1.2*** notes, whatever maximizes $L(\theta)$ is the same input that maximizes $L(g(\theta))$. Apparently this is due to the properties of probability set functions made rigorous by something called "induced probability measure" or "pushforward measure" where a transformation of a random variable carries over the properties of the original random variable it transforms to preserve the probabilities. But it is important to distinguish this property in probability set functions and likelihood functions from generic real functions (i.e. just because $x_o$ maximizes $f(x)$ does not mean $f(g(x_o))$ maximizes $f$ !)

***Theorem 6.1.3*** then demonstrates the the parameter where the derivative of the likelihood function is $0$ (i.e. the mle), is consistent predictor of the true parameter. The theorem demonstrates that the mle has at least one desirable quality.
# Definitions
