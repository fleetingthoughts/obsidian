...
***Definition (Linear Dependence).*** A subset, $S$, of vector space $V$ over some field $F$ will be called linearly dependent if there exist a finite number of distinct vectors $u_1,u_2,...u_n \in S$ and at least one non-zero scalars $a_1,a_2,...a_n$ such that $$a_1u_1 + a_2u_2+...+a_nu_n = 0$$
