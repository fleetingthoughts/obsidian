...
1) How to determine the expectation of a linear combination of random variables in terms of the expectation of the summand random variables.
2) How to determine the variance and covariance of linear combination of random variables in terms of the variance of the summand random variables.

The entirety of this chapter looks at random variables, $T$, that are linear combinations of other random variables: 
$$T = \Sigma^{n}_{i=1}a_iX_i$$We can completely determine its mean and variance if we know the mean and variance of the random variables $X_i$. ***Theorem 2.8.1*** shows that the expectation of $T$ is the same linear combination of the expectation which is proven by the additivity of the integral and linearity of expectations.


***Theorem 2.8.2. (Covariance of a linear combination of random variables).*** Given another random variable, $W$, that is a linear combination of random variables $W = \Sigma_{i}^{n}b_iY_i$ , if $E(X_{i}^2)$ and $E(Y_{i}^2)$ are both finite [then by a theorem, their means must exist](Introduction%20to%20Mathematical%20Statistics%20-%201.10%20Important%20Inequalities.md) and we can determine $Cov(T,W)$
...
***Corollary 1 of Theorem 2.8.2 (Variance of a linear combination of random variables)*** The variance of $T$ is simply the covariance with itself:
...
***Corollary 2 of Theorem 2.8.2 (Variance of a linear combination of independent random variables)***
$$Var(T) = \Sigma_{i}^{n} a_{i}^{2}\sigma_{i}^2$$
Our motivation to relate a random variable to its linear combination is to analyze random samples. ***Definition 2.8.1*** defines random samples as a set of random variables that are identically distributed and independent from each other. The assumption of both identical distribution and independence is common when sampling randomly from a population. Its important to realize two things about random samples:
