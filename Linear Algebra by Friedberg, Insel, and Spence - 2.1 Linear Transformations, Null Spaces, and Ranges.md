...
What ***Theorem 2.6*** means is when defining a linear transformation, we do not have to give a general rule for what it does for an arbitrary vector $v\in V$ in the domain; we can also say how it maps the specific basis vectors and be assured that it is a linear transformation and it is the only one that has the rule we just assigned. ***Theorem 2.6*** is general because it applies to any $n$ vectors in the codomain, but we could alternatively try to map to the basis of $W$. There is only one linear transformation that maps the basis of $V$ to the basis of $W$ and in doing so, it completely maps $W$ with domain $V$. 
